...
export class AudioSession {
  ...
  async demo() {
    const id = this.begin("Pulse study · 30 seconds");
    try {
      const ctx = this.context();
      await ctx.resume();
      if (!this.isCurrent(id)) return;
      const buffer = ctx.createBuffer(1, ctx.sampleRate * 30, ctx.sampleRate);
      const out = buffer.getChannelData(0);
      for (let i = 0; i < out.length; i++) {
        const t = i / ctx.sampleRate;
        const beat = t % 0.5;
        const tick = t % 0.25;
        const fade = Math.min(1, t * 4, (30 - t) * 2);
        out[i] =
          fade *
          (0.35 *
            Math.sin(
              2 * Math.PI * (48 * beat + 9 * (1 - Math.exp(-beat * 24))),
            ) *
            Math.exp(-beat * 14) +
            0.07 * Math.sin(2 * Math.PI * 660 * t) * Math.exp(-tick * 30) +
            0.04 *
