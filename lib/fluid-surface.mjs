...
export function magneticCells() {
  ...
  for (let i = 0; i < CELL_COUNT; i++) {
    ...
    const x = Math.cos(i * golden) * radius + 0.085 * Math.sin(i * 17.13);
    const z = Math.sin(i * golden) * radius + 0.085 * Math.cos(i * 11.71);
    const yy = y + 0.075 * Math.sin(i * 7.37);
...
// four compact polynomial lobes per vertex instead of a 64-cell search.
