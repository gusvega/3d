...
export class FluidResponse {
  ...
  update(data, sampleRate, dt, settings = {}) {
    ...
    let total = 0,
      ...
      high = 0,
      flux = 0;
    for (let band = 0; band < CELL_COUNT; band++) {
      const lowHz = 35 * Math.pow(12000 / 35, band / CELL_COUNT);
      const highHz = 35 * Math.pow(12000 / 35, (band + 1) / CELL_COUNT);
      let value = 0;
      ...
      flux += Math.max(0, value - this.previous[band]) / CELL_COUNT;
      ...
      const modeGain =
        settings.mode === "bass"
          ...
          : settings.mode === "detail"
            ...
            : 1;
      const idle =
        field * (0.32 + 0.12 * Math.sin(this.time * 0.45 + seed * 20));
      const activity = this.levels[band] * gain * modeGain;
      const target = clamp(
        idle + activity * 0.58 + this.onset * (0.07 + seed * 0.13),
        0,
        0.86,
      );
