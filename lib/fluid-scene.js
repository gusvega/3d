...
const surfaceGLSL = `
attribute vec4 cellOwners;
uniform vec4 magneticCells[${CELL_COUNT}];
...
vec4 fluidShape(vec3 n) {
  ...
  float radius = 1.43;
  for (int j = 0; j < 4; j++) {
    int index = int(cellOwners[j]);
    vec4 cell = magneticCells[index];
    float cosine = dot(n, cell.xyz);
    float seed = fract(sin(float(index) * 127.1 + 19.7) * 43758.5453);
    float width = mix(0.010, 0.018, seed);
    float exponent = mix(2.3, 4.8, fract(seed * 13.7));
    float q = max(0.0, (cosine - 1.0 + width) / width);
    float lobe = pow(q, exponent);
    radius += cell.w * lobe;
    gradient += cell.w * exponent * pow(q, exponent - 1.0) / width * (cell.xyz - n * cosine);
  }
  vec3 axis = vec3(2.8, 1.7, -2.2);
  float phase = dot(n, axis) + fluidTime * 0.48;
  float amp = 0.13 + fluidEnergy * 0.10;
  radius += sin(phase) * amp;
  gradient += cos(phase) * amp * (axis - n * dot(n, axis));
  // Broad, intersecting currents break the spherical silhouette without
  // introducing seams. Their exact gradients preserve smooth reflections.
  vec3 flowAxis = vec3(-1.6, 3.1, 1.2);
  float flow = dot(n, flowAxis) - fluidTime * 0.31;
  float flowAmp = 0.095 + fluidEnergy * 0.055;
  radius += cos(flow) * flowAmp;
  gradient -= sin(flow) * flowAmp * (flowAxis - n * dot(n, flowAxis));
  vec3 foldAxis = vec3(5.1, 2.3, -3.7);
  float fold = dot(n, foldAxis) + sin(fluidTime * 0.24);
  radius += sin(fold) * 0.033;
  gradient += cos(fold) * 0.033 * (foldAxis - n * dot(n, foldAxis));
...
function studioEnvironment(renderer) {
  ...
  studio.background = new THREE.Color(0.1, 0.1, 0.1);
  const panels = [];
  ...
  panel(3.5, 7, new THREE.Color(6.1, 6.1, 6.1), [-5, 3, 4]);
  panel(7, 2, new THREE.Color(5.2, 5.2, 5.2), [0, 6, -1]);
  panel(1.1, 8, new THREE.Color(3.4, 3.4, 3.4), [5, 1, 2]);
  panel(6, 4, new THREE.Color(0.8, 0.8, 0.8), [0, -5, 3]);
  panel(3, 5, new THREE.Color(2.0, 2.0, 2.0), [-3, 0, -6]);
...
export function createFluidScene(canvas, getAudio, getSettings) {
  ...
  // Sphere vertices are immutable. Only the cell amplitudes cross the CPU/GPU
  ...
  material.customProgramCacheKey = () => "ferrofluid-dense-v3";
  ...
  function animate(now) {
    ...
    if (settings.finish !== lastMaterial) {
      ...
      material.color.set(settings.finish === "mercury" ? 0xdcdcdc : 0x828282);
    ...
    if (dirty && (paused || now - lastDraw >= 1000 / 65)) {
      ...
      if (now - metricTime > 250 || paused || !canvas.dataset.triangles) {
