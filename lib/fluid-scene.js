...
export function createFluidScene(canvas, getAudio, getSettings) {
  ...
  function animate(now) {
    ...
    if (dirty && (paused || now - lastDraw >= 1000 / 65)) {
      ...
      if (!paused) {
        slowFrames =
          cost > 20 || dt > 0.03 ? slowFrames + 1 : Math.max(0, slowFrames - 1);
        geometrySlowFrames =
          dt > 0.03
            ? geometrySlowFrames + 1
            : Math.max(0, geometrySlowFrames - 1);
        fastFrames = cost < 9 && dt < 0.02 ? fastFrames + 1 : 0;
        if (slowFrames > 35 && pixelRatio > 1) {
          pixelRatio = Math.max(1, pixelRatio - 0.2);
          resize();
          slowFrames = 0;
        }
        if (geometrySlowFrames > 70 && qualityTier < resolutions.length - 1) {
          qualityTier++;
          const old = geometry;
